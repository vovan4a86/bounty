...
document.addEventListener('DOMContentLoaded', function () {
  var links = document.querySelectorAll('.tinymodal-modal'); //ОБРАБОТКА ВСЕХ СОБЫТИЙ МОДАЛЬНЫХ ОКОН (КРОМЕ ПОСЛЕ ОТПРАВКИ ФОРМЫ)
  ...
  }); //ПОЯВЛЕНИЕ КОНОПКИ GOTOP после скроллинга 200px сверху
  ...
  var serverErrText = document.querySelector('#order-server-err'); //ВАЛИДАЦИЯ ФОРМЫ
  ...
  form.addEventListener('input', function () {
    ...
    Array.from(form.elements).forEach(function (input) {
      input.addEventListener('click', function () {
        serverErrText.innerHTML = ''; //очищаем ошибки при клике на форму
      });
